<?php
    ...
    elseif(!empty($_GET['antwort'])) {
        //da der get-wert von antwort ein string ist, konvertieren in INT
...
<html>
    ...
    <body>
        ...
        <p>Frage: <?php echo $_SESSION['karteikarten'][''.$_SESSION['fragennr'].'']['frage']; ?></p>
        
        <p id="antwort">Antwort : <?php echo $_SESSION['karteikarten'][''.$_SESSION['fragennr'].'']['antwort']; ?></p>
        <button id="bt_antwort_einblenden" onclick="blendeAntwortEin();">Antwort einblenden</button>
        <br>

        <form action="lernen_1.php" method="GET">
        ...
        <?php } ?>
        <!--TODO -- weiter['frage vor'] muss auf fragenpool begrenzt werden/ abhaengig von kategorie-->
        <?php if ($_SESSION['button_weiter']) { ?>
        <a href="?frage=<?=$_SESSION['frage_vor']?>"> weiter zu Frage <?=$_SESSION['frage_vor']?> &nbsp;</a>
        <?php } ?>
        
        <a href="auswertung_1.php"> Auswertung </a>
